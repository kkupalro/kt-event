							...
							<c:otherwise>
								<div class="scope">
									<c:if test="${map.pageIdx eq 0}">
										<a href="javascript:;" class="dir first disabled">첫 페이지로
											이동</a>
										<a href="javascript:;" class="dir prev disabled">이전 페이지로
											이동</a>
									</c:if>
											...
											<c:choose>
												...
												<c:when test="${map.pageIdx >= 5}">
													...
													<fmt:parseNumber var="val" integerOnly="true"
														value="${pages*5 + 4<map.endPage?4:map.endPage - pages*5}" />
													<c:forEach begin="${pages*5}" end="${pages*5 + val}"
														var="idx">
														<a id="${idx}"
															href="javascript:kt_common.ktMenuLinkStat('page?pageIdx=${idx}&searchOption=${map.searchOption}&searchType=${map.searchType}','^${idx}페이지','_self','');">${idx+1}</a>
													</c:forEach>
									...
									<c:if test="${map.pageIdx eq map.endPage}">
										<a href="javascript:;" class="dir next disabled">다음 페이지로
											이동</a>
