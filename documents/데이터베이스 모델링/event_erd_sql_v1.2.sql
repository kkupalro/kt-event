...
CREATE TABLE `cust_dt` (
  `CUST_ID` int NOT NULL AUTO_INCREMENT COMMENT '회원 아이디',
  `PAY_SYS` varchar(255) NOT NULL COMMENT '요금제',
  `MBS` varchar(255) NOT NULL DEFAULT '화이트' COMMENT '멤버쉽',
  `M_POINT` int NOT NULL DEFAULT '50000' COMMENT '포인트',
  `DATA_SIZE` double NOT NULL DEFAULT '0' COMMENT '데이터 용량',
  PRIMARY KEY (`CUST_ID`),
  CONSTRAINT `cust_dt_ibfk_1` FOREIGN KEY (`CUST_ID`) REFERENCES `cust_bas` (`CUST_ID`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='회원 상세정보';
